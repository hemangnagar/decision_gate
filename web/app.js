...
function badge(materiality, count) {
  ...
  return `<span class="metric ${cls}">${count} ${materiality.replace('_',' ')}</span>`;
}

function render(ledger) {
  $('decisionText').textContent = ledger.decision;
  $('claims').innerHTML = ledger.claims.map(c => `<div><b>${c.id}</b> ${c.title}<small>${c.statement}</small></div>`).join('');
  ...
  $('challenges').innerHTML = ledger.challenges.map(c => `
    <details><summary><span class="pill ${c.materiality.toLowerCase()}">${c.materiality}</span> ${c.title}</summary>
      <p>${c.argument}</p><p><b>Resolves if:</b> ${c.resolves_if}</p><p><b>Target:</b> ${c.target_claim}</p>
    </details>`).join('');
  ...
  $('nextAction').innerHTML = unresolved.length ? `<b>What to do next</b>${unresolved.map(c => `<div>${c.resolves_if}</div>`).join('')}` : '<b>Review closed.</b> Proceed with the accepted risks recorded above.';
